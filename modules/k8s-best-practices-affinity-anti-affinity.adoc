...
**Impact of not following this practice:** Missing affinity rules can cause incorrect pod placement, leading to performance issues and failure to meet co-location requirements.
====
...
**Impact of not following this practice:** Missing anti-affinity rules can cause all pod replicas to be scheduled on the same node, creating single points of failure.
====
